            ...
            while True:
                ...
                try:
                    decrypted_message = hybrid_decrypt(
                        encrypted_aes_key, iv, encrypted_message, "/app/Bob_private_key.pem"
                    )
...
def reverse_self_delete_email(email_id, port):
    ...
    try:
        ...
        with open("userdata.log", "a") as log_file:
            ...
            log_file.write(f"{datetime.now()} | Action: reverse soft delete email | Email ID: {email_id}\n")
    except Exception as e:
        print(f"Error: {e}")
    finally:
        if client_socket:
...
def manage_emails_menu(port):
    while True:
        ...
        print("6. Reverse soft delete an email")
        print("7. Exit")
